...
void Texture::Texture::init(const std::string& fileName)
{
...
void Texture::controlledBind(GLuint unit, GLuint handle)
{
	assert(unit >= 0 && unit <= 31); /// check we are working with one of the 32 textures

	glActiveTexture(GL_TEXTURE0 + unit); //set acitve texture unit
	glBindTexture(GL_TEXTURE_2D, handle); //type of and texture to bind to unit
}
