...
void main()
{
    ...
    noiseColour *= vec3(0.8, 0.8, 0.8);
    noiseColour = pow(noiseColour, vec3(3));
    noiseColour += vec3(0.0, 0.0, 0.1);
    ...
    // Diffuse lighting
    vec3 diffuseNormal = normalize(vert_In.Normal * texture(normal, vert_In.tC).rgb);
    ...
    float specularStrength = max(dot(reflection, camera), 0.0);
    specularStrength = pow(specularStrength, 256.0);
    vec3 specular = specularStrength * lightColour;

    // Final lighting
    vec3 lighting = ambient * 0.2 + diffuseColor * 1.0 + specular * 1.0;

    // Set colour
    vec3 modelColour = texture(diffuse, vert_In.tC + (time * 0.03)).rgb;
    vec3 phongColour = modelColour * (lighting + 2);
